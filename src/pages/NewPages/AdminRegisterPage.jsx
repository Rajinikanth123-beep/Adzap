...
export default function AdminRegisterPage({ onRegister, onNavigate, registrationsClosed = false }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (registrationsClosed) {
      setError('Admin registrations are closed. Maximum 6 admins reached.');
      return;
    }
    setError('');
    const result = onRegister(formData);
    const success = typeof result === 'boolean' ? result : result?.success;
    const message = typeof result === 'boolean' ? 'Admin with this email already exists' : result?.message;

    if (!success) {
      setError(message || 'Admin registration failed');
      return;
    }
  ...
  return (
    <div className="login-page">
      <div className="login-container">
        ...
        <p className="login-subtitle">Create a separate admin account</p>
        {registrationsClosed && (
          <div className="error-message">Admin registrations are closed. Maximum 6 admins reached.</div>
        )}
        ...
        <form onSubmit={handleSubmit} className="login-form">
          ...
          <button type="submit" className="btn btn-submit" disabled={registrationsClosed}>
            Register Admin
          </button>

          <div className="login-footer">
            <button type="button" onClick={() => onNavigate('admin-login')} className="link-btn">
